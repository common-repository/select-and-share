...
function _ss_shared_network(network, ss_selected_text) {
  ...
  switch(network) {
    ...
    case 'email':
      ...
      break;
    case 'wordpress':
      _ss_share_wordpress(ss_selected_text);
      break;
  }
...
function _ss_share_wordpress(ss_selected_text) {
  var wordpress = prompt(_ss_wordpress_text, "http://");
  if(wordpress != null && wordpress != "" && wordpress != "http://") {
    if(wordpress.charAt(wordpress.length - 1) != '/') { wordpress = wordpress + '/' }
    _ss_window_open(wordpress + 'wp-admin/press-this.php?u=' + _ss_url_site + '&t=' + _ss_title_page + '&s=' + ss_selected_text + '&v=1', 950, 600);
  }
}

function _ss_share_email(ss_selected_text) {
  var email = prompt(_ss_email_text, "");
  if(email != null && email != "") {
    window.location = 'mailto:' + email + '?subject=' + _ss_title_page + '&body=' + ss_selected_text + escape('\n\n') + escape(_ss_url_site);
  }
}
